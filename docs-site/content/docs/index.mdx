...
import { DocsHome, DocsHintStrip } from '@/components/docs-home';

<DocsHome />

<p className="slash-label">Documentation</p>
...
Custom themes can be added via TOML files in `/usr/share/sysc-greet/themes/` or `~/.config/sysc-greet/themes/`. See [Themes Configuration](configuration/themes) for details.
...
For detailed configuration, see [Wallpapers Feature](features/wallpapers).
...
For more details, see [ASCII Art Feature](features/ascii-art).
...
- [Niri Setup](compositors/niri) — default
- [Cagebreak Setup](compositors/cagebreak) — the Hyprland replacement
- [Sway Setup](compositors/sway)
- [Hyprland Setup](compositors/hyprland) — deprecated
...
For development details, see [Development Guide](development/architecture).
